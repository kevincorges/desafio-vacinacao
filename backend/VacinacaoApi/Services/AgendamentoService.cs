...
namespace VacinacaoApi.Services
{
    ...
    public class AgendamentoService : IAgendamentoService
    {
        ...
        public async Task<string> Criar(CriarAgendamentoDto dto)
        {
            ...
            // Regra: Intervalo de 1 hora entre agendamentos
            var agendamentosProximos = await _context.Agendamentos
                .Where(a => a.DataAgendamento.Date == dataApenas)
                .ToListAsync();

            foreach (var agendamentoExistente in agendamentosProximos)
            {
                var diferenca = Math.Abs((agendamentoExistente.HorarioAgendamento - horario).TotalMinutes);
                if (diferenca > 0 && diferenca < 60)
                    return "Deve haver um intervalo de 1 hora entre agendamentos.";
            }
